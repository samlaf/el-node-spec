...
Developers will publish their packages following the format defined [below](#node-package) in a separate git repository (Tap) called `eigen-<avs name>`, which involves developers initializing a git repository with the required and standard structure that will be defined later. The [AVS setup wizard](../wizard/intro) would rely on this structure to get package information and install the Node.

Developers manage these repositories, and with each new release, they update the tap with the corresponding configuration files. The new package will only be considered correct if it follows the required structure and if a git tag is created with it in the tap repository.
...
```
tool add some/avs -v v1.0.0 [profile] [options...]
```
